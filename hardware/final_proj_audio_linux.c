...
int run_server(void)
{
	...
	while (!gs_quit) {  // sit inside this while checking for new data from the DSP until TCPIP connection disconnected
		...
		if  (GPIO_getOutput(mygpio,DATAFORFILE_TO_LINUX_BANK,DATAFORFILE_TO_LINUX_FLAG) == 0) { // New data from DSP ? 
			...
			// get the audio clip code from cache
			char audioClipCode = (char)myshared->scratch[0];
			char sysCallBuffer = char[512];
			// put appropriate audio script path into sysCallBuffer
			switch(audioClipCode) {
				case 'b':
					sprintf(sysCallBuffer,"/home/root/wavfiles/bwang.sh");
				break;
			}
			// print out our command, then execute it
			printf("%s\n", sysCallBuffer);
			system(sysCallBuffer);
			
			// set flag indicating to DSP that Linux is ready for more data
